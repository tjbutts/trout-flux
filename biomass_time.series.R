...
arrows(x0=fish.final$year, y0=log(fish.final$c.se.l), 
       x1=fish.final$year, y1=log(fish.final$c.se.u), col = '#99000d', code = 3, angle=90, length=0, lwd = 2)
...
# MIV plot # =============================
plot(log(chaoborus)~year4, data = pmiv.final, type = 'o',col = '#99000d', lwd = 2, pch = 19, cex = 1, ylim = c(log(0.001),log(300)),
     ylab = '', xlab = '', yaxt = 'n') 
axis(side=2,
...
arrows(x0=pmiv.final$year4, y0=(log(pmiv.final$chaoborus - pmiv.final$chaoborus.se)), 
       x1=pmiv.final$year4, y1=(log(pmiv.final$chaoborus + pmiv.final$chaoborus.se)), col = '#99000d', code = 3, angle=90, length=0, lwd = 2)
points(log(leptodora)~year4, data = pmiv.final, type = 'o', col = '#ef3b2c', lwd = 2, pch = 19, cex = 1)
arrows(x0=pmiv.final$year4, y0=(log(pmiv.final$leptodora - pmiv.final$leptodora.se)), col = '#ef3b2c',
       x1=pmiv.final$year4, y1=(log(pmiv.final$leptodora + pmiv.final$leptodora.se)), code = 3, angle=90, length=0, lwd = 2)
points(log(mysis)~year4, data = pmiv.final, type = 'o', col = '#fc9272', lwd = 2, pch = 19, cex = 1)
arrows(x0=pmiv.final$year4, y0=(log(pmiv.final$mysis - pmiv.final$mysis.se)), col = '#fc9272',
       x1=pmiv.final$year4, y1=(log(pmiv.final$mysis + pmiv.final$mysis.se)), code = 3, angle=90, length=0, lwd = 2)
points(log(bythotrephes)~year4, data = pmiv.final, type = 'o', col = 'gray40', lwd = 2, pch = 19, cex = 1)
arrows(x0=pmiv.final$year4, y0=(log(pmiv.final$bythotrephes - pmiv.final$bytho.se)), col = 'gray40',
       x1=pmiv.final$year4, y1=(log(pmiv.final$bythotrephes + pmiv.final$bytho.se)), code = 3, angle=90, length=0, lwd = 2)

legend('bottomleft', legend = c('Chaoborus', 'Leptodora', 'Mysis', 'Bythotrephes'), pch = 19, col = c('#99000d','#ef3b2c', '#fc9272', 'gray40'),
       bty = 'n', cex = 1.5)
abline(v = 2006.5, lty = 2)
abline(v = 2014.5, lty = 2)
...
arrows(x0=zp.final$year, y0=(log(zp.final$cladocera - zp.final$clad.se)), 
       x1=zp.final$year, y1=(log(zp.final$cladocera + zp.final$clad.se)), col = '#99000d', code = 3, angle=90, length=0, lwd = 2)
points(log(copepoda)~year, data = zp.final, type = 'o', col = '#ef3b2c', lwd = 2, pch = 19, cex = 1)
arrows(x0=zp.final$year, y0=(log(zp.final$copepoda - zp.final$cope.se)), col = '#ef3b2c',
       x1=zp.final$year, y1=(log(zp.final$copepoda + zp.final$cope.se)), code = 3, angle=90, length=0, lwd = 2)
points(log(rotifera)~year, data = zp.final, type = 'o', col = '#fc9272', lwd = 2, pch = 19, cex = 1)
arrows(x0=zp.final$year, y0=(log(zp.final$rotifera - zp.final$roti.se)), col = '#fc9272',
       x1=zp.final$year, y1=(log(zp.final$rotifera + zp.final$roti.se)), code = 3, angle=90, length=0, lwd = 2)

legend('bottomleft', legend = c('Cladocera', 'Copepoda', 'Rotifera'), pch = 19, col = c('#99000d','#ef3b2c', '#fc9272'),
       bty = 'n', cex = 1.5)
abline(v = 2006.5, lty = 2)
abline(v = 2014.5, lty = 2)

# ug per L 
# plot(log(cladocera)~year, data = zp.wide.check, type = 'o',col = '#99000d', lwd = 2, pch = 19, cex = 1, ylim = c(log(1),log(8000)),
#      ylab = '', xlab = '', yaxt = 'n') 
# axis(side=2,
#      at=c(log(0.1), log(0.2),log(0.3),log(0.4),log(0.5),log(0.6),log(0.7),log(0.8),log(0.9),
#           log(1), log(2),log(3),log(4),log(5),log(6),log(7),log(8),log(9),log(10),
...
#      las=0)
# mtext(side = 2, line =2.5, expression(Log~`[`~Zoop~ ~Biomass ~`(`~ mg ~ m^-2~`)]`))
# points(log(copepoda)~year, data = zp.wide.check, type = 'o', col = '#ef3b2c', lwd = 2, pch = 19, cex = 1)
# 
# points(log(rotifera)~year, data = zp.wide.check, type = 'o', col = '#fc9272', lwd = 2, pch = 19, cex = 1)
# 
# legend('bottomleft', legend = c('Cladocera', 'Copepoda', 'Rotifera'), pch = 19, col = c('#99000d','#ef3b2c', '#fc9272'),
#        bty = 'n', cex = 1.5)
# abline(v = 2006.5, lty = 2)
