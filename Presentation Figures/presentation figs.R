...
library(ggplot2)
ggplot(tfp, aes( y = pref.tf, x = year)) + 
...
# Cisco -> fish # 
plot(log(flux_kJ_d)~year, type = 'o', pch = 19, lwd =2, col = transparent, ylim = c(log(90), log(300000000)), yaxt = 'n', 
...
# Cisco -> fish # 
flux.1_adjust = flux.1.preference %>%
  filter(year < 2015)
points(log(flux_kJ_d)~year, type = 'o', pch = 19, lwd =2, col = f.1, ylim = c(log(90), log(300000000)), yaxt = 'n', 
...
flux.2_adjust = flux.2.preference %>% 
  filter(year )
# pMIV -> LT & CSC# 
points(log(flux_kJ_d)~year, type = 'o', pch = 15, lwd =2, col = f.2, cex = 2,
